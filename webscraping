...
window = Tk()
window.title("Extractor de Emails y Teléfonos")
...
tab_control.add(tab2,text="Extraer desde Url/Links")
tab_control.add(tab3,text='Acerca de')

tab_control.pack(expand=1,fill='both')


label1 = Label(tab1, text= 'Extractor de Emails y Teléfonos',padx=5, pady=5)
label1.grid(column=0, row=0)
 
label2 = Label(tab2, text= 'Extractor URL',padx=5, pady=5)
label2.grid(column=0, row=0)

label3 = Label(tab3, text= 'Acerca de',padx=5, pady=5)
...
def extract_phonenumbers():
	...
	num_of_results = len(final_extract)
	result = '\nCantidad de teléfonos:{},\nNúmero:{}'.format(num_of_results,final_extract)
	tab1_display.insert(tk.END,result)
...
# Principal
l1=Label(tab1,text="Ingresar textos para extracción")
...
# BOTONES
button_1=Button(tab1,text="Resetear",command=clear_text, width=10,bg='#03A9F4',fg='#fff')
...
button_2=Button(tab1,text="Teléfono",command=extract_phonenumbers, width=10,bg='green',fg='#fff')
button_2.grid(row=5,column=1,padx=10,pady=10)

button_3=Button(tab1,text="Limpiar resultados", command=clear_display_result,width=10,bg='#03A9F4',fg='#fff')
...
# para extraer desde Web Links
l1=Label(tab2,text="Ingresar texto para extraer links")
l1.grid(row=1,column=0)
...
# Acerca de
about_label = Label(tab3,text="Herramienta WebScraping V.0.0.1 \n OperDev",pady=5,padx=5)
